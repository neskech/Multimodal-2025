...
if USE_LAION:
    CACHE_FILE = f"data/clip_embeddings_cache_laion_sample_{model_name.replace('/', '-')}.npz"
elif USE_COCO:
...
def load_embeddings():
    ...
    if os.path.exists(CACHE_FILE):
        ...
        print(f"Cache file {CACHE_FILE} not found. Please run cardelph_projection.py first to generate embeddings.")
...
def generate_incremental_components(max_components, max_fits):
    ...
    components = list(range(1, max(20, max_components) + 1, max_components//max_fits))  # Dense for small values
...
def fit_gmm_with_model_selection(X, max_components=10):
    ...
    print(f"Fitting GMM with up to {max_components} components using incremental approach...")
    
    # Generate incremental component sequence
    component_sequence = generate_incremental_components(max_components, MAX_TOTAL_FITS)
    
    print(f"Testing {len(component_sequence)} different component counts: {component_sequence[:10]}{'...' if len(component_sequence) > 10 else ''}")
    ...
    for i, n_components in enumerate(component_sequence):
        print(f"Fitting GMM {i+1}/{len(component_sequence)}: {n_components} components...")
...
def visualize_gmm_results(X, gmm, labels_true=None, title="GMM Clustering Results"):
    ...
    scatter = axes[0].scatter(X_2d[:, 0], X_2d[:, 1], c=labels_pred, cmap='viridis', alpha=0.6)
    axes[0].set_title(f'{title}\nCluster Assignments ({gmm.n_components} components)')
    axes[0].set_xlabel(f'PC1 ({pca.explained_variance_ratio_[0]:.2%} var)' if X.shape[1] > 2 else 'Dim 1')
    axes[0].set_ylabel(f'PC2 ({pca.explained_variance_ratio_[1]:.2%} var)' if X.shape[1] > 2 else 'Dim 2')
    plt.colorbar(scatter, ax=axes[0])
    
    # Plot 2: Uncertainty (entropy of probabilities)
    entropy = -np.sum(proba * np.log(proba + 1e-10), axis=1)
    scatter2 = axes[1].scatter(X_2d[:, 0], X_2d[:, 1], c=entropy, cmap='plasma', alpha=0.6)
    axes[1].set_title('Assignment Uncertainty\n(Higher = More Uncertain)')
    axes[1].set_xlabel(f'PC1 ({pca.explained_variance_ratio_[0]:.2%} var)' if X.shape[1] > 2 else 'Dim 1')
    axes[1].set_ylabel(f'PC2 ({pca.explained_variance_ratio_[1]:.2%} var)' if X.shape[1] > 2 else 'Dim 2')
...
def print_model_selection_summary(results):
    ...
    print("="*80)
    print(f"{'Components':<12} {'Log-Lik':<12} {'AIC':<12} {'BIC':<12} {'Silhouette':<12} {'Converged':<12}")
    print("-"*80)
    ...
    for i in indices_to_show:
        ...
        print(f"{components:<12} {log_lik:<12.2f} {aic:<12.1f} {bic:<12.1f} {silhouette_str:<12} {converged:<12} {marker_str}")
        prev_idx = i
    
    print("-"*80)
    print(f"Tested {total_results} different component counts using incremental approach")
...
def analyze_embeddings_by_type(embeddings, labels, types, embedding_type, max_components=MAX_N_COMPONENTS):
    ...
    if len(X) > SAMPLE_SIZE:
        print(f"Sampling {SAMPLE_SIZE} {embedding_type} embeddings from {len(X)} total...")
    ...
    results, models = fit_gmm_with_model_selection(X_normalized, max_components=MAX_N_COMPONENTS)
    
    if not results:
        print(f"No GMM models were successfully fitted for {embedding_type} embeddings.")
        return None, None
    ...
    visualize_gmm_results(X_normalized, best_gmm, filtered_labels, 
...
def compare_image_text_statistics(embeddings, types):
    ...
    print("-" * 40)
    print(f"{'Mean norm (original)':<25} {np.mean(image_norms.flatten()):<15.4f} {np.mean(text_norms.flatten()):<15.4f}")
    print(f"{'Std norm (original)':<25} {np.std(image_norms.flatten()):<15.4f} {np.std(text_norms.flatten()):<15.4f}")
    print(f"{'Mean (normalized)':<25} {np.mean(image_normalized):<15.4f} {np.mean(text_normalized):<15.4f}")
    print(f"{'Std (normalized)':<25} {np.std(image_normalized):<15.4f} {np.std(text_normalized):<15.4f}")
    ...
    image_sample = image_normalized[np.random.choice(len(image_normalized), sample_size, replace=False)]
    text_sample = text_normalized[np.random.choice(len(text_normalized), sample_size, replace=False)]
    
    image_distances = pdist(image_sample, metric='cosine')
    text_distances = pdist(text_sample, metric='cosine')
    
    print(f"{'Mean cosine distance':<25} {np.mean(image_distances):<15.4f} {np.mean(text_distances):<15.4f}")
    print(f"{'Std cosine distance':<25} {np.std(image_distances):<15.4f} {np.std(text_distances):<15.4f}")
...
def main():
    ...
    # Visualize the best model
    visualize_gmm_results(X_normalized, best_gmm, sampled_labels, 
    ...
    if image_results and text_results:
        ...
        print("-" * 62)
        print(f"{'Image Only':<20} {image_results[image_best_idx]['n_components']:<12} {image_results[image_best_idx]['bic']:<15.1f} {image_results[image_best_idx]['log_likelihood']:<15.2f}")
        print(f"{'Text Only':<20} {text_results[text_best_idx]['n_components']:<12} {text_results[text_best_idx]['bic']:<15.1f} {text_results[text_best_idx]['log_likelihood']:<15.2f}")
        print(f"{'Combined':<20} {results[combined_best_idx]['n_components']:<12} {results[combined_best_idx]['bic']:<15.1f} {results[combined_best_idx]['log_likelihood']:<15.2f}")
