...
class VonMisesFisher:
    ...
    def sample(self, n_samples: int) -> np.ndarray:
        ...
        if len(samples) < n_samples:
            warnings.warn(
                f"Only generated {len(samples)} samples out of {n_samples} requested"
            )
    ...
    def _sample_high_concentration(self, n_samples: int) -> np.ndarray:
        ...
        for _ in range(n_samples):
            ...
            perturbation = np.random.randn(self.dimension) * (
                1.0 / np.sqrt(self.concentration)
            )
    ...
    def _sample_fallback(self, n_samples: int) -> np.ndarray:
        ...
        indices = np.random.choice(
            len(candidates), size=n_samples, p=probabilities, replace=True
        )
...
class VonMisesFisherMixture:
    ...
    def sample(self, n_samples: int) -> np.ndarray:
        ...
        component_indices = np.random.choice(
            len(self.components), size=n_samples, p=self.weights
        )
