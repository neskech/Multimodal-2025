...
def evaluate_mixture_model(mixture_model: VonMisesFisherMixture, 
    ...
    component_stats = []
    for i, (weight, mean_dir, conc) in enumerate(zip(weights, mean_directions, concentrations)):
...
def save_results(mixture_model: VonMisesFisherMixture, 
    ...
    if convergence_info['log_likelihood_history']:
        ...
        plt.grid(True)
        plt.savefig(os.path.join(output_dir, 'convergence_plot.png'), dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    parser = argparse.ArgumentParser(description='Fit von Mises-Fisher mixture model to CLIP embeddings')
    ...
    print(f"Embedding norms (should be ~1.0): mean={np.mean(np.linalg.norm(embeddings, axis=1)):.4f}, std={np.std(np.linalg.norm(embeddings, axis=1)):.4f}")
    ...
    print(f"EM parameters: max_iterations={args.max_iterations}, tolerance={args.tolerance}")
    ...
    for stats in evaluation_metrics['component_stats']:
        print(f"  Component {stats['component_id']}: weight={stats['weight']:.4f}, concentration={stats['concentration']:.4f}")
