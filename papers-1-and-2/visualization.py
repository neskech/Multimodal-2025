...
class VonMisesFisherVisualizer:
    ...
    def __init__(self, mixture_model: VonMisesFisherMixture):
        ...
        self.weights, self.mean_directions, self.concentrations = mixture_model.get_parameters()
    ...
    def plot_mean_directions_2d(self, data: Optional[np.ndarray] = None, 
                               method: str = 'pca', figsize: Tuple[int, int] = (10, 8)):
        """
        ...
        if data_2d is not None:
            ax.scatter(data_2d[:, 0], data_2d[:, 1], alpha=0.3, s=20, 
                      c='lightblue', label='Data points')
        
        # Plot mean directions
        colors = plt.cm.Set3(np.linspace(0, 1, len(self.mean_directions)))
        for i, (mean_dir_2d, weight, conc) in enumerate(zip(mean_dirs_2d, self.weights, self.concentrations)):
            # Size proportional to weight, color intensity to concentration
            size = weight * 1000
            alpha = min(1.0, conc / 10.0)  # Normalize concentration for alpha
            
            ax.scatter(mean_dir_2d[0], mean_dir_2d[1], 
                      s=size, c=[colors[i]], alpha=alpha, 
    ...
    def plot_mean_directions_3d(self, data: Optional[np.ndarray] = None, 
        ...
        if data is not None:
            ax.scatter(data[:, 0], data[:, 1], data[:, 2], 
                      alpha=0.2, s=10, c='lightblue', label='Data points')
        
        # Plot mean directions
        colors = plt.cm.Set3(np.linspace(0, 1, len(self.mean_directions)))
        for i, (mean_dir, weight, conc) in enumerate(zip(self.mean_directions, self.weights, self.concentrations)):
            # Size proportional to weight
            size = weight * 1000
            
            ax.scatter(mean_dir[0], mean_dir[1], mean_dir[2],
    ...
    def plot_data_assignment(self, data: np.ndarray, method: str = 'pca',
        ...
        assignment_counts = np.bincount(assignments, minlength=self.mixture_model.n_components)
    ...
    def create_comprehensive_visualization(self, data: Optional[np.ndarray] = None,
        ...
        for i, (mean_dir_2d, weight) in enumerate(zip(mean_dirs_2d, self.weights)):
            size = weight * 1000
            ax3.scatter(mean_dir_2d[0], mean_dir_2d[1], s=size, c=[colors[i]], 
        ...
        if data is not None:
            ...
            for k in range(self.mixture_model.n_components):
                mask = assignments == k
                if np.any(mask):
                    ax6.scatter(data_2d[mask, 0], data_2d[mask, 1], 
                               c=[colors[k]], alpha=0.6, s=20)
...
def visualize_mixture_model(mixture_model: VonMisesFisherMixture, 
    ...
    fig1.savefig(os.path.join(output_dir, 'component_parameters.png'), dpi=300, bbox_inches='tight')
    plt.close(fig1)
    
    print("Creating mean directions 2D plot...")
    fig2 = visualizer.plot_mean_directions_2d(data, method='pca')
    fig2.savefig(os.path.join(output_dir, 'mean_directions_2d_pca.png'), dpi=300, bbox_inches='tight')
    plt.close(fig2)
    
    if mixture_model.dimension >= 3:
        print("Creating mean directions 3D plot...")
        fig3 = visualizer.plot_mean_directions_3d(data)
        fig3.savefig(os.path.join(output_dir, 'mean_directions_3d.png'), dpi=300, bbox_inches='tight')
        plt.close(fig3)
    
    print("Creating similarity matrix plot...")
    fig4 = visualizer.plot_component_similarity_matrix()
    fig4.savefig(os.path.join(output_dir, 'similarity_matrix.png'), dpi=300, bbox_inches='tight')
    plt.close(fig4)
    
    if data is not None:
        print("Creating data assignment plot...")
        fig5 = visualizer.plot_data_assignment(data, method='pca')
        fig5.savefig(os.path.join(output_dir, 'data_assignments.png'), dpi=300, bbox_inches='tight')
        plt.close(fig5)
    
    print("Creating component entropy plot...")
    fig6 = visualizer.plot_component_entropy()
    fig6.savefig(os.path.join(output_dir, 'component_entropies.png'), dpi=300, bbox_inches='tight')
    plt.close(fig6)
    
    print("Creating comprehensive visualization...")
    fig7 = visualizer.create_comprehensive_visualization(data)
    fig7.savefig(os.path.join(output_dir, 'comprehensive_visualization.png'), dpi=300, bbox_inches='tight')
    plt.close(fig7)
