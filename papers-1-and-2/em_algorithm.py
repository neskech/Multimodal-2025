...
class VonMisesFisherEM:
    ...
    def _m_step_mean_directions(
        self, data: np.ndarray, responsibilities: np.ndarray
    ) -> np.ndarray:
        ...
        for k in range(self.n_components):
            ...
            if norm > 1e-10:
                ...
                mean_directions[k] = mean_directions[k] / np.linalg.norm(
                    mean_directions[k]
                )

        return mean_directions

    def _m_step_concentrations(
        self,
        data: np.ndarray,
        ...
        for k in range(self.n_components):
            ...
            weighted_dot_product = np.sum(
                responsibilities[:, k] * dot_products
    ...
    def fit(self, data: np.ndarray, verbose: bool = True) -> VonMisesFisherMixture:
        ...
        for iteration in range(self.max_iterations):
            ...
            responsibilities = self._e_step(
                data, weights, mean_directions, concentrations
            )

            # M-step
            new_weights = self._m_step_weights(responsibilities)
            new_mean_directions = self._m_step_mean_directions(data, responsibilities)
            new_concentrations = self._m_step_concentrations(
                data, responsibilities, new_mean_directions
            )

            # Compute log-likelihood
            log_likelihood = self._compute_log_likelihood(
                data, new_weights, new_mean_directions, new_concentrations
            )
            ...
            if verbose and (iteration + 1) % 10 == 0:
                print(
                    f"Iteration {iteration + 1}: Log-likelihood = {log_likelihood:.4f}"
                )
    ...
    def get_convergence_info(self) -> dict:
        ...
        return {
            ...
            "final_log_likelihood": (
                self.log_likelihood_history[-1] if self.log_likelihood_history else None
            ),
            "log_likelihood_history": self.log_likelihood_history,
        }


def fit_von_mises_fisher_mixture(
    data: np.ndarray,
    n_components: int,
    max_iterations: int = 100,
    tolerance: float = 1e-6,
    random_state: Optional[int] = None,
    verbose: bool = True,
) -> Tuple[VonMisesFisherMixture, dict]:
    """
