    ...
    def extract_image_embeddings(self, image_paths: List[str]) -> np.ndarray:
        ...
        for image_path in image_paths:
            try:
                ...
                embeddings.append(np.zeros((1, 512)))  # CLIP ViT-B/32 has 512-dim embeddings
        
        return np.vstack(embeddings)
    
    def extract_multimodal_embeddings(self, texts: List[str] = None, 
                                    image_paths: List[str] = None) -> np.ndarray:
        """
...
def create_sample_data(extractor: CLIPEmbeddingExtractor, 
