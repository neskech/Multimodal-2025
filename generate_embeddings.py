...
# Try to import both CLIP implementations
try:
    import clip  # Original OpenAI CLIP
    CLIP_AVAILABLE = True
except ImportError:
    CLIP_AVAILABLE = False
    
try:
    import open_clip  # OpenCLIP for more models
...
class EmbeddingGenerator:
    ...
    def _load_openclip_model(self):
        ...
        self.model, _, self.preprocess = open_clip.create_model_and_transforms(
            self.model_name,
            pretrained=self.pretrained_name or "openai",
            device=self.device
        )
    ...
    def generate_text_embeddings(self, texts: List[str]) -> np.ndarray:
        ...
        for i in tqdm(range(0, len(texts), batch_size), desc="Processing text batches"):
            ...
            if self.use_openclip:
                ...
                text_tokens = self.tokenizer(batch_texts, truncate=True).to(self.device)
...
def main():
    parser = argparse.ArgumentParser(description="Generate CLIP embeddings for various datasets")
    ...
    parser.add_argument("--custom_path", type=str, help="Path to custom images dir or text file")
    ...
    parser.add_argument("--pretrained_name", type=str, help="Pretrained checkpoint name (for OpenCLIP)")
    parser.add_argument("--use_openclip", action="store_true", help="Use OpenCLIP instead of original CLIP")
    ...
    # Processing options
    parser.add_argument("--include_images", action="store_true", default=True, help="Include image embeddings")
    parser.add_argument("--include_texts", action="store_true", default=True, help="Include text embeddings")
    ...
    for item in data:
        if args.include_texts and item["text"]:
            ...
            labels.append(str(item.get("image_id", item.get("text_id", item.get("sample_id", "unknown")))))
    ...
    if all_embeddings:
        ...
        raise ValueError("No embeddings generated. Check your dataset and include options.")
